...
export default function LocationsPage() {
  ...
  const handleCreate = async () => {
    ...
    await locationsApi.create({ name: newLocation, description: newDescription || undefined })
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="animate-fade-in-up">
        ...
        <h1 className="text-4xl font-serif text-foreground">Ubicaciones</h1>
        <p className="text-muted-foreground mt-1">Gestiona las ubicaciones de almacenamiento</p>
      </div>

      {/* Create Form */}
      <Card className="border-border/50 card-luxury animate-fade-in-up stagger-1 opacity-0">
        <div className="p-6 border-b border-border/30">
          <h2 className="text-lg font-serif">Nueva Ubicación</h2>
        </div>
        <div className="p-6">
      ...
      {loading ? (
              ...
              <CardContent className="p-5">
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex gap-1">
                    ...
                    <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg" onClick={() => handleDelete(location.id)}>
        ...
        <div className="flex flex-col items-center justify-center min-h-[300px] animate-fade-in">
          ...
          <p className="text-muted-foreground">No hay ubicaciones. ¡Crea la primera!</p>
        </div>
      )}
    </div>
  )

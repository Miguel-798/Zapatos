...
from app.application.dto.shoe_dto import ShoeDetailDTO, ShoeSizeOutputDTO
...
class GetShoeUseCase:
    """Use case for getting a shoe by ID"""
    
    def __init__(self, shoe_repository: ShoeRepository):
        self.shoe_repository = shoe_repository
    
    async def execute(self, shoe_id: UUID) -> ShoeDetailDTO:
        shoe = await self.shoe_repository.get_by_id(shoe_id)
        if not shoe:

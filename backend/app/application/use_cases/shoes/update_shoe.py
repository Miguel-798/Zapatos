...
from app.application.dto.shoe_dto import UpdateShoeDTO, ShoeDetailDTO, ShoeSizeOutputDTO
...
class UpdateShoeUseCase:
    """Use case for updating an existing shoe"""
    
    def __init__(self, shoe_repository: ShoeRepository):
        self.shoe_repository = shoe_repository
    
    async def execute(self, shoe_id: UUID, dto: UpdateShoeDTO) -> ShoeDetailDTO:
        shoe = await self.shoe_repository.update(shoe_id, dto)
        if not shoe:
            raise ValueError(f"Zapato no encontrado: {shoe_id}")
        
        # Fetch sizes, colors, materials, and names
        sizes_data = await self._get_shoe_sizes(shoe_id)
        colors_data = await self._get_shoe_colors(shoe_id)
        materials_data = await self._get_shoe_materials(shoe_id)
